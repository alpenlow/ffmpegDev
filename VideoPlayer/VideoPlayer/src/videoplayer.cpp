...
static int audio_decode_frame(VideoState *is, double *pts_ptr) {
    ...
    for(;;) {
        while(is->audio_pkt_size > 0) {
            ...
            } else av_frame_unref(is->audio_frame);
            ...
            if(is->audio_frame->format != is->audio_src_fmt || dec_channel_layout != is->audio_src_channel_layout ||
                    is->audio_frame->sample_rate != is->audio_src_freq ||
                    (wanted_nb_samples != is->audio_frame->nb_samples && !is->swr_ctx)) {
                if(is->swr_ctx) swr_free(&is->swr_ctx);

                is->swr_ctx = swr_alloc_set_opts(NULL, is->audio_tgt_channel_layout, (AVSampleFormat)is->audio_tgt_fmt,
                    is->audio_tgt_freq, dec_channel_layout, (AVSampleFormat)is->audio_frame->format,
            ...
            if(is->swr_ctx) {
                ...
                if(wanted_nb_samples != is->audio_frame->nb_samples) {
                    if(swr_set_compensation(is->swr_ctx, (wanted_nb_samples - is->audio_frame->nb_samples) *
                            is->audio_frame->sample_rate / is->audio_frame->sample_rate, wanted_nb_samples *
                ...
                len2 = swr_convert(is->swr_ctx, out, sizeof(is->audio_buf2) / is->audio_tgt_channels /
                        av_get_bytes_per_sample(is->audio_tgt_fmt), in, is->audio_frame->nb_samples);
        ...
        if(packet_queue_get(&is->audioq, pkt, 0) <= 0) return -1;
